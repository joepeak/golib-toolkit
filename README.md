...
```bash
# 安装完整工具集
go get github.com/joepeak/golib-util
...
import (
    ...
    "github.com/joepeak/golib-util/database"
...
func main() {
    ...
    // 获取缓存
    cachedUser, err := cache.Get(ctx, "user:1")
    if err != nil {
...
func main() {
    ...
    log.Println("Work completed, releasing lock")
}
```
...
import (
    ...
    "github.com/joepeak/golib-util/redisclient"
    _ "github.com/joepeak/golib-conf"
)
...
import (
    ...
    "github.com/joepeak/golib-util/rand"
    "github.com/joepeak/golib-util/snowflake"
...
- **数据库**: 连接池管理，支持读写分离
- **Redis 缓存**: 高性能缓存，支持防缓存击穿、热键检测、指标统计
- **分布式锁**: 基于 Redis 的可靠分布式锁，支持自动续期和批量操作
- **对象存储**: 断点续传，并发上传
- **工具函数**: 高性能算法，内存优化
...
### v0.5.0
- 🚀 **新增 Redis 包**: 高性能缓存和分布式锁功能
- ✨ 完整的缓存功能: 基础操作、批量操作、模式匹配
- 🔐 分布式锁: 支持自动续期、批量操作、锁管理器
- 🛡️ 高级特性: 防缓存击穿、热键检测、指标统计
- 📊 泛型支持: 类型安全的缓存操作
- 🔧 灵活配置: 丰富的配置选项和自定义配置
- 📚 完善文档: 详细的使用示例和最佳实践
